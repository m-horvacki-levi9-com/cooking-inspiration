...
describe('App', () => {
  ...
  it('renders compact cards from search endpoint data without ingredients or method steps', async () => {
  ...
  it('lazy-loads recipe details only when opening expanded view and renders ingredients with method steps', async () => {
    const user = userEvent.setup();
    ...
    expect(within(dialog).getByRole('list', { name: 'Method steps' })).toBeInTheDocument();
  ...
  it('preserves Bring! affordance and uses canonical detail URL in expanded recipe actions', async () => {
    ...
    expect(screen.getAllByRole('link', { name: 'Import to Bring!' })).toHaveLength(1);
